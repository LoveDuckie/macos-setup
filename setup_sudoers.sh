...
is_sudoers_configured() {
    if [ -e /etc/sudoers.d/$USER ] && [[ "$(cat /etc/sudoers.d/$(whoami))" == *"$(whoami)"* ]]; then
        return 0
    fi
    return 1
}
...
configure_sudoers() {
    if is_sudoers_configured; then
        write_warning "setup-sudoers" "sudoers already configured (\"/etc/sudoers.d/$(whoami)\")"
        exit 1
    fi
...
if ! configure_sudoers; then
    write_error "setup-shared-sudoers" "failed to configure sudoers"
    exit 1
fi
